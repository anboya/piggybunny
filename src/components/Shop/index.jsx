...
const requireContext = require.context("../../uploads/Products", true, /^\.\/.*\.jpg$/);
...
export default class Gallery extends Component {
    ...
    onChangeCheck = list => {
        this.setState({ checkedList: list, indeterminate: (!!list.length && list.length < plainOptions.length), checkAll:(list.length === plainOptions.length)})
    }
    onCheckAllChange = e => {
    ...
    render() {
        ...
        const newdata = sortedData.filter(item => this.state.checkedList.includes(item.kind))
        return (
            <div className='shop' style={{ marginBottom: 100 }}>
                ...
                <Row >
                    <Col className="checkbox" >
                        ...
                        <CheckboxGroup options={plainOptions} value={this.state.checkedList} onChange={this.onChangeCheck} />
                        ...
                        <Radio.Group  onChange={this.onChangeRadio} value={value}>
                            ...
                            <Radio value={3}>Price: High to High</Radio>
                            </Space>
                        </Radio.Group>
                    </Col>
                    <Col flex={1} className="shopList">
                    <div style={{height:30,backgroundColor:'#f2ce63',color:'#fff',marginBottom:50,textAlign:'center',borderRadius:10}}>The below products are just for display</div>    
                        <List grid={{ gutter: 32, column: 4 }} dataSource={newdata} renderItem={item => (
                                <List.Item >
                                    <Card hoverable bordered={false} style={{padding:0}} >
                                        <Image  style={{objectFit:'fill',borderTopLeftRadius:10,borderTopRightRadius:10}} src={item.img} />
